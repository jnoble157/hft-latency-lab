...
if [[ ${EUID:-$(id -u)} -ne 0 ]]; then
  echo "[ERROR] Must run as root: sudo ${BASH_SOURCE[0]} <pci> [<pci> ...] [--driver=vfio-pci|uio_pci_generic]" >&2
  exit 1
fi
...
if [[ ${#args[@]} -lt 1 ]]; then
  echo "[USAGE] sudo ${BASH_SOURCE[0]} 0000:05:00.0 [0000:05:00.1] [--driver=vfio-pci]" >&2
  exit 2
fi
